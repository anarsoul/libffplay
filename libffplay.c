...
static int64_t sws_flags = SWS_FAST_BILINEAR;
...
typedef struct VideoState {
	...
	enum ShowMode {
		SHOW_MODE_NONE = -1, SHOW_MODE_VIDEO =
		    0, SHOW_MODE_WAVES, SHOW_MODE_RDFT, SHOW_MODE_NB
	} show_mode;
	...
	libffplay_picture_t pictq[VIDEO_PICTURE_QUEUE_SIZE];
	int pictq_size, pictq_rindex, pictq_windex;
	pthread_mutex_t pictq_mutex;
	pthread_cond_t pictq_cond;
	struct SwsContext *img_convert_ctx;
	libffplay_rect_t last_display_rect;
			...
			int y, int w, int h, int color, int update)
{
	...
	fill_rectangle(screen,
		       xleft + width - w2, ytop, w2, height, color, update);
	fill_rectangle(screen,
		       xleft + w1, ytop, width - w1 - w2, h1, color, update);
	fill_rectangle(screen,
		       xleft + w1, ytop + height - h2,
		       width - w1 - w2, h2, color, update);
}
#endif
...
static void free_picture(VideoState *is, libffplay_picture_t * vp)
{
	if (vp->allocated)
		is->videomgr->free_picture_planes(is->videomgr, vp);
}
	...
	if (vp->allocated) {
		...
		if (rect.x != is->last_display_rect.x
			...
			fill_border(is->xleft, is->ytop, is->width, is->height,
				    rect.x, rect.y, rect.w, rect.h, bgcolor, 1);
...
static void video_audio_display(VideoState * s)
{
	...
	if (!s->paused) {
		int data_used =
		    s->show_mode == SHOW_MODE_WAVES ? s->width : (2 * nb_freq);
		n = 2 * channels;
	...
	if (s->show_mode == SHOW_MODE_WAVES) {
		...
		if (rdft_bits != s->rdft_bits) {
			...
			s->rdft_data =
			    av_malloc(4 * nb_freq * sizeof(*s->rdft_data));
		}
		{
			FFTSample *data[2];
			for (ch = 0; ch < nb_display_channels; ch++) {
				...
				for (x = 0; x < 2 * nb_freq; x++) {
					double w =
					    (x - nb_freq) * (1.0 / nb_freq);
					data[ch][x] =
					    s->sample_array[i] * (1.0 - w * w);
					i += channels;
					if (i >= SAMPLE_ARRAY_SIZE)
						i -= SAMPLE_ARRAY_SIZE;
				}
...
#if 0
static void set_default_window_size(libffplay_picture_t * vp)
{
	libffplay_rect_t rect;
...
static void stream_toggle_pause(VideoState * is)
{
	if (is->paused) {
		is->frame_timer +=
		    av_gettime() / 1000000.0 + is->vidclk.pts_drift -
		    is->vidclk.pts;
...
static double vp_duration(VideoState * is, libffplay_picture_t * vp,
			  libffplay_picture_t * nextvp)
{
...
{
	libffplay_picture_t *prevvp;
	int ret = 0;
	/* update queue size and signal for the previous picture */
	prevvp =
	    &is->pictq[(is->pictq_rindex + VIDEO_PICTURE_QUEUE_SIZE - 1) %
			...
			if (is->subtitle_st) {
				while (is->subpq_size > 0) {
					...
					if (sp->serial != is->subtitleq.serial
						...
						pthread_mutex_lock(&is->
								   subpq_mutex);
						is->subpq_size--;
						pthread_cond_signal(&is->
								    subpq_cond);
	...
	if (show_status) {
		...
		if (!last_time || (cur_time - last_time) >= 30000) {
			...
			av_log(NULL, AV_LOG_INFO,
			       ...
			       PRId64 "/%" PRId64 "   \r", get_master_clock(is),
			       (is->audio_st
				&& is->video_st) ? "A-V" : (is->video_st ? "M-V"
							    : (is->audio_st ?
							       "M-A" : "   ")),
			       av_diff,
			       is->frame_drops_early + is->frame_drops_late,
			       aqsize / 1024, vqsize / 1024, sqsize,
			       is->video_st ? is->video_st->
			       codec->pts_correction_num_faulty_dts : 0,
			       is->video_st ? is->video_st->
			       codec->pts_correction_num_faulty_pts : 0);
			fflush(stdout);
			last_time = cur_time;
		}
	}
...
static void alloc_picture(VideoState * is)
{
	...
	bufferdiff =
	    vp->bmp ? FFMAX(vp->bmp->pixels[0],
			    vp->bmp->pixels[1]) - FFMIN(vp->bmp->pixels[0],
							vp->bmp->pixels[1]) : 0;
...
static void duplicate_right_border_pixels(SDL_Overlay * bmp)
{
	...
	for (i = 0; i < 3; i++) {
		...
		if (bmp->pitches[i] > width) {
			...
			for (p = bmp->pixels[i] + width - 1; p < maxp;
			     p += bmp->pitches[i])
				*(p + 1) = *p;
		}
	}
			 ...
			 double duration, int64_t pos, int serial)
{
	...
	vp->sar_den = src_frame->sample_aspect_ratio.den;
	vp->sar_num = src_frame->sample_aspect_ratio.num;

	/* alloc or resize hardware picture buffer */
	if (!vp->allocated) {
		res = is->videomgr->alloc_picture_planes(is->videomgr, vp,
	...
	is->videomgr->lock_picture(is->videomgr, vp);

	pict.data[0] = vp->planes[0];
	pict.data[1] = vp->planes[1];
	pict.data[2] = vp->planes[2];

	pict.linesize[0] = vp->pitches[0];
	pict.linesize[1] = vp->pitches[1];
	pict.linesize[2] = vp->pitches[2];
	...
	sws_scale(is->img_convert_ctx, (const uint8_t * const *)src_frame->data,
		  src_frame->linesize, 0, src_frame->height, pict.data,
		  pict.linesize);

	/* workaround SDL PITCH_WORKAROUND */
	/* FIXME: duplicate_right_border_pixels(vp->bmp); */
	/* update the bitmap content */
	is->videomgr->unlock_picture(is->videomgr, vp);
			   ...
			   int *serial)
{
	...
	if (pkt->data == NULL && pkt->size == 0 && !is->audio_st
	    && !is->eof_event_sent) {
		is->eof_event_sent = 1;
				 ...
				 AVFilterContext * sink_ctx)
{
	...
	if (filtergraph) {
		...
		if ((ret =
		     avfilter_graph_parse_ptr(graph, filtergraph, &inputs,
					      &outputs, NULL)) < 0)
			goto fail;
	} else {
				   ...
				   const char *vfilters, AVFrame * frame)
{
	...
	if ((ret =
	     av_opt_set_int_list(filt_out, "pix_fmts", pix_fmts,
	...
	if ((ret =
	     configure_filtergraph(graph, vfilters, filt_src, filt_crop)) < 0)
		goto fail;
				   ...
				   int force_output_format)
{
	...
	ret = snprintf(asrc_args, sizeof(asrc_args),
		       ...
		       is->audio_filter_src.channels, 1,
		       is->audio_filter_src.freq);
	...
	if ((ret =
		...
		goto end;
	if ((ret =
	     av_opt_set_int(filt_asink, "all_channel_counts", 1,
			    AV_OPT_SEARCH_CHILDREN)) < 0)
		goto end;
	...
	if (force_output_format) {
		...
		if ((ret =
		     av_opt_set_int(filt_asink, "all_channel_counts", 0,
				    AV_OPT_SEARCH_CHILDREN)) < 0)
			goto end;
		if ((ret =
			...
			goto end;
		if ((ret =
		     av_opt_set_int_list(filt_asink, "channel_counts", channels,
					 -1, AV_OPT_SEARCH_CHILDREN)) < 0)
			goto end;
		if ((ret =
		     av_opt_set_int_list(filt_asink, "sample_rates",
					 sample_rates, -1,
					 AV_OPT_SEARCH_CHILDREN)) < 0)
			goto end;
	}
	...
	if ((ret =
	     configure_filtergraph(is->agraph, afilters, filt_asrc,
				   filt_asink)) < 0)
		goto end;
...
static void *video_thread(void *arg)
{
	...
	for (;;) {
		...
		if (last_w != frame->width
		    || last_h != frame->height
		    || last_format != frame->format || last_serial != serial) {
			av_log(NULL, AV_LOG_DEBUG,
			       "Video frame changed from size:%dx%d format:%s serial:%d to size:%dx%d format:%s serial:%d\n",
			       last_w, last_h, (const char *)
			       av_x_if_null(av_get_pix_fmt_name(last_format),
					    "none"), last_serial, frame->width,
			       frame->height, (const char *)
			       av_x_if_null(av_get_pix_fmt_name(frame->format),
					    "none"), serial);
			avfilter_graph_free(&graph);
			graph = avfilter_graph_alloc();
			if ((ret =
			     configure_video_filters(graph, is, vfilters,
						     frame)) < 0) {
				if (!is->eof_event_sent) {
					is->eof_event_sent = 1;
					is->eventmgr->send_event(is->eventmgr,
								 LFP_EVENT_EOF,
								 NULL);
				}
				goto the_end;
		...
		while (ret >= 0) {
			...
			if (fabs(is->frame_last_filter_delay) >
			    AV_NOSYNC_THRESHOLD / 10.0)
				is->frame_last_filter_delay = 0;
			tb = filt_out->inputs[0]->time_base;
#endif
			duration = (frame_rate.num
				    && frame_rate.den ? av_q2d((AVRational) {
							       frame_rate.den,
							       frame_rate.num}
				    ) : 0);
			pts =
			    (frame->pts ==
			     AV_NOPTS_VALUE) ? NAN : frame->pts * av_q2d(tb);
			ret =
			    queue_picture(is, frame, pts, duration,
					  av_frame_get_pkt_pos(frame), serial);
			av_frame_unref(frame);
#if 0
		}
#endif
	...
	for (;;) {
		...
		if (got_subtitle && sp->sub.format == 0) {
			...
			for (i = 0; i < sp->sub.num_rects; i++) {
				for (j = 0; j < sp->sub.rects[i]->nb_colors;
				     j++) {
					RGBA_IN(r, g, b, a,
						(uint32_t *) sp->sub.
						rects[i]->pict.data[1] + j);
					y = RGB_TO_Y_CCIR(r, g, b);
					u = RGB_TO_U_CCIR(r, g, b, 0);
					v = RGB_TO_V_CCIR(r, g, b, 0);
					YUVA_OUT((uint32_t *) sp->sub.
						 rects[i]->pict.data[1] + j, y,
						 u, v, a);
				}
			}
	...
	for (;;) {
		...
		while (pkt_temp->stream_index != -1
			...
			if (!is->audio_buf_frames_pending) {
				len1 =
				    avcodec_decode_audio4(dec, is->frame,
							  &got_frame, pkt_temp);
				if (len1 < 0) {
				...
				tb = (AVRational) {
				1, is->frame->sample_rate};
				if (is->frame->pts != AV_NOPTS_VALUE)
					is->frame->pts =
					    av_rescale_q(is->frame->pts,
							 dec->time_base, tb);
				else if (is->frame->pkt_pts != AV_NOPTS_VALUE)
					is->frame->pts =
					    av_rescale_q(is->frame->pkt_pts,
							 is->
							 audio_st->time_base,
							 tb);
				else if (is->audio_frame_next_pts !=
					 AV_NOPTS_VALUE)
					is->frame->pts =
					    av_rescale_q
					    (is->audio_frame_next_pts,
					     (AVRational) {
				...
				if (reconfigure) {
					...
					av_get_channel_layout_string(buf1,
								     sizeof
								     (buf1), -1,
								     is->audio_filter_src.channel_layout);
					av_get_channel_layout_string(buf2,
								     sizeof
								     (buf2), -1,
								     dec_channel_layout);
					av_log(NULL, AV_LOG_DEBUG,
					       ...
					       (is->audio_filter_src.fmt), buf1,
					       is->audio_last_serial,
					       is->frame->sample_rate,
					       av_frame_get_channels(is->frame),
					       av_get_sample_fmt_name
					       (is->frame->format), buf2,
					       is->audio_pkt_temp_serial);

					is->audio_filter_src.fmt =
					    is->frame->format;
					is->audio_filter_src.channels =
					    av_frame_get_channels(is->frame);
					is->audio_filter_src.channel_layout =
				...
				if ((ret =
				     av_buffersrc_add_frame(is->in_audio_filter,
							    is->frame)) < 0)
					return ret;
			}
			if ((ret =
			     av_buffersink_get_frame_flags(is->out_audio_filter,
							   is->frame, 0)) < 0) {
			...
			dec_channel_layout =
			    ...
			    ? is->
			    frame->channel_layout :
			    av_get_default_channel_layout(av_frame_get_channels
							  (is->frame));
			wanted_nb_samples =
			    synchronize_audio(is, is->frame->nb_samples);
			...
			if (is->swr_ctx) {
				...
				av_fast_malloc(&is->audio_buf1,
					       &is->audio_buf1_size, out_size);
				if (!is->audio_buf1)
					return AVERROR(ENOMEM);
				len2 =
				    swr_convert(is->swr_ctx, out, out_count, in,
						is->frame->nb_samples);
				if (len2 < 0) {
					av_log(NULL, AV_LOG_ERROR,
					       "swr_convert() failed\n");
		...
		/* read next packet */
		if ((packet_queue_get
		     (&is->audioq, pkt, 1, &is->audio_pkt_temp_serial)) < 0)
			return -1;

		/* XXX: is it right time to do it? */
		if (pkt->data == NULL && pkt->size == 0 && !is->eof_event_sent) {
			is->eof_event_sent = 1;
			is->eventmgr->send_event(is->eventmgr, LFP_EVENT_EOF,
						 NULL);
		}
		...
		if (pkt->data == flush_pkt.data) {
			...
			if ((is->ic->
			     iformat->flags & (AVFMT_NOBINSEARCH |
					       AVFMT_NOGENSEARCH |
					       AVFMT_NO_BYTE_SEEK))
...
static void libffplay_audio_callback(void *opaque, void *stream, size_t len)
{
	VideoState *is = opaque;
	int audio_size, len1;
	double ts = libffplay_tell(opaque);

	audio_callback_time = av_gettime();

	is->eventmgr->send_event(is->eventmgr, LFP_EVENT_TIMESTAMP,
				 (void *)&ts);
		      ...
		      struct AudioParams *audio_hw_params)
{
	...
	/* TODO: using stereo, 16 bit for now */
	res = is->audiomgr->open_device(is->audiomgr, wanted_sample_rate, 2,
					16, opaque, libffplay_audio_callback);
	if (res < 0) {
		av_log(NULL, AV_LOG_ERROR, "Failed to open audio device: %d\n",
		       res);
		return res;
	}
	...
	audio_hw_params->channel_layout = av_get_default_channel_layout(2);
	audio_hw_params->channels = 2;
	audio_hw_params->frame_size =
	    av_samples_get_buffer_size(NULL, audio_hw_params->channels, 1,
				       audio_hw_params->fmt, 1);
	audio_hw_params->bytes_per_sec =
	    av_samples_get_buffer_size(NULL, audio_hw_params->channels,
	...
	case AVMEDIA_TYPE_AUDIO:
		{
			...
			is->audio_filter_src.channel_layout =
			    get_valid_channel_layout(avctx->channel_layout,
						     avctx->channels);
			is->audio_filter_src.fmt = avctx->sample_fmt;
			if ((ret =
			     configure_audio_filters(is, afilters, 0)) < 0)
				return ret;
		...
		if ((ret =
		     audio_open(is, channel_layout, nb_channels, sample_rate,
				&is->audio_tgt)) < 0)
			return ret;
		...
		is->audio_diff_threshold =
		    2.0 * is->audio_hw_buf_size / is->audio_tgt.bytes_per_sec;
...
static void stream_component_close(VideoState * is, int stream_index)
{
	...
	case AVMEDIA_TYPE_AUDIO:
		...
		is->audiomgr->close_device(is->audiomgr);
...
static void *read_thread(void *arg)
{
	...
	if (start_time != AV_NOPTS_VALUE) {
		...
		ret =
		    avformat_seek_file(ic, -1, INT64_MIN, timestamp, INT64_MAX,
				       0);
		if (ret < 0) {
			av_log(NULL, AV_LOG_WARNING,
	...
	for (;;) {
...
#if 0				/* CONFIG_RTSP_DEMUXER || CONFIG_MMSH_PROTOCOL */
		if (is->paused &&
		...
		if (is->seek_req) {
			...
			int64_t seek_min =
			    is->seek_rel >
			    0 ? seek_target - is->seek_rel + 2 : INT64_MIN;
			int64_t seek_max =
			    is->seek_rel <
			    0 ? seek_target - is->seek_rel - 2 : INT64_MAX;
// FIXME the +-2 is due to rounding being not done in the correct direction in generation
//      of the seek_pos/seek_rel variables

			ret =
			    avformat_seek_file(is->ic, -1, seek_min,
					       seek_target, seek_max,
					       is->seek_flags);
			if (ret < 0) {
				av_log(NULL, AV_LOG_ERROR,
		...
		if (infinite_buffer < 1 &&
		    (is->audioq.size + is->videoq.size + is->subtitleq.size >
		     ...
		     ((is->audioq.nb_packets > MIN_FRAMES
		       || is->audio_stream < 0 || is->audioq.abort_request)
		      && (is->videoq.nb_packets > MIN_FRAMES
			  || is->video_stream < 0 || is->videoq.abort_request
			  || (is->
			      video_st->disposition &
		...
		if (!is->paused &&
		    ...
		    && (!is->video_st
			|| (is->video_finished == is->videoq.serial
			    && is->pictq_size == 0))) {
			if (loop != 1 && (!loop || --loop)) {
				stream_seek(is,
					    start_time !=
					    AV_NOPTS_VALUE ? start_time : 0, 0,
					    0);
			} else if (autoexit) {
				ret = AVERROR_EOF;
				goto fail;
		...
		if (eof) {
			if (is->video_stream >= 0)
				packet_queue_put_nullpacket(&is->videoq,
							    is->video_stream);
			if (is->audio_stream >= 0)
				packet_queue_put_nullpacket(&is->audioq,
							    is->audio_stream);
			if (is->subtitle_stream >= 0)
		...
		pkt_in_play_range = duration == AV_NOPTS_VALUE ||
		    ...
		    (double)(start_time !=
			     AV_NOPTS_VALUE ? start_time : 0) / 1000000 <=
...
int stream_open(struct VideoState *is, const char *filename,
		AVInputFormat * iformat)
{
...
static void stream_cycle_channel(VideoState * is, int codec_type)
{
	...
	if (codec_type != AVMEDIA_TYPE_VIDEO && is->video_stream != -1) {
		...
		if (p) {
			...
			for (start_index = 0; start_index < nb_streams;
			     start_index++)
				if (p->stream_index[start_index] ==
				    stream_index)
					break;
...
static void refresh_loop_wait_event(VideoState * is, SDL_Event * event)
{
	...
	while (!SDL_PeepEvents(event, 1, SDL_GETEVENT, SDL_ALLEVENTS)) {
		if (!cursor_hidden
		    && av_gettime() - cursor_last_shown > CURSOR_HIDE_DELAY) {
			SDL_ShowCursor(0);
			cursor_hidden = 1;
		}
...
static void seek_chapter(VideoState * is, int incr)
{
	...
	for (i = 0; i < is->ic->nb_chapters; i++) {
		...
		if (av_compare_ts(pos, AV_TIME_BASE_Q, ch->start, ch->time_base)
		    < 0) {
			i--;
			break;
		}
...
void libffplay_seek(libffplay_ctx_t * ctx, double wanted_pos, int whence)
{
	VideoState *is = (struct VideoState *)ctx;
	double incr, pos;
	if (seek_by_bytes) {
		/* TODO */
		av_log(NULL, AV_LOG_ERROR,
		       "Stream seek by bytes is not supported yet!\n");
	} else {
